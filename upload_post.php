...
// ----------------------------------------------------------------------------------
   ...
   if (!empty($_FILES["headlineimage"]["name"])) {
      ...
      $q = "SELECT * FROM headlines WHERE id = '" . $id . "'";
      $r = mysqli_query($con, $q) or die(mysqli_error($con));  
      $rows = mysqli_fetch_array($r, MYSQLI_ASSOC);

      $path = "headlines_image/".$rows['headline_image'];
      unlink($path);
...
// -----------------------------------------------------------------------
   		$query1 = "INSERT INTO post (title,author,category,tags,date,blog,upload_time,post_url,code,source,image_name,brief,type) VALUES('" . $title . "','". $author ."','". $category ."','". $tags ."','" . $date . "','" . $blog . "','". $upload_time ."','" . $post_url . "','" . $code . "','" . $source . "','" . $image_name . "','" . $brief . "','" . $type . "')";
   		mysqli_query($con, $query1) or die(mysqli_error($con));
